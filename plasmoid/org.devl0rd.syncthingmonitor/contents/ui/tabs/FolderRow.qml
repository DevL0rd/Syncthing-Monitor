...
Rectangle {
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                PlasmaComponents.Label {
                    text: row.query !== "" ? Highlight.mark(row.folder.label || row.folderId, row.query, Kirigami.Theme.highlightColor) : row.folder.label || row.folderId
        ...
        Loader {
            ...
            sourceComponent: Component {
                ColumnLayout {
                    ...
                    ColumnLayout {
                        visible: row.folderState === "busy" && (root.folderNeedLoading[row.folderId] === true || row.queueItems.length > 0)
                        Layout.fillWidth: true
                        spacing: 3
                        ...
                        Repeater {
                            model: row.queueItems.length
                            RowLayout {
                                required property int index
                                readonly property var modelData: row.queueItems[index] || ({})
                                Layout.fillWidth: true
                                spacing: Kirigami.Units.smallSpacing
                                Rectangle {
        ...
        Loader {
            Layout.fillWidth: true
            active: row.errors.length > 0 && (row.expanded || row.query !== "")
            visible: active
            sourceComponent: Component {
                ColumnLayout {
                    Layout.fillWidth: true
                    ...
                    Repeater {
                        model: Math.min(6, row.errors.length)
                        ColumnLayout {
                            required property int index
                            readonly property var modelData: row.errors[index] || ({})
                            Layout.fillWidth: true
                            spacing: 0
                            PlasmaComponents.Label {
                                text: row.query !== "" ? Highlight.mark(modelData.path, row.query, Kirigami.Theme.highlightColor) : modelData.path
                                textFormat: row.query !== "" ? Text.StyledText : Text.PlainText
                                font: Kirigami.Theme.smallFont
                                elide: Text.ElideMiddle
                                Layout.fillWidth: true
